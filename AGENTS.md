...
This repo stores cross-agent skills under root-level `skills/<skill-name>/SKILL.md`. At the start of a task, agents that support local file access (Codex, Claude, Gemini, or other CLIs) should scan `skills/*/SKILL.md` frontmatter and descriptions, then load the matching skill body when the user's request matches its trigger. Treat these root `skills/` entries as project-local workflow contracts, independent of any vendor-specific global skill registry. When a skill references reusable code, prefer the repo `scripts/` path named by the skill.
...
For portfolio-report-family runs that render daily decision sections, follow the `horizon_v1` / `research_targets` contract in `docs/portfolio_report_agent_guidelines/04-computations-to-static-snapshot.md`: derive expected horizon lots-first from snapshot lots, reject generic headline-only support, prefer primary/official sources, and keep Phase A research metadata separate from Phase B investment judgment. For `portfolio_report`, skip that live research layer by policy unless a future rendered section explicitly opts in. For ad hoc single-name notes, inherit the same decision-grade standard when judging a ticker — short-term reads need a tactical state (`act_now` / `wait` / `exit` / `need_data`), mid/long reads need thesis or strategic status, and source gaps need explicit audit — but do **not** require portfolio-report JSON schema or `research_targets` outside report runs.
...
Choose both report axes before running: `--report-type <daily_report|portfolio_report>` and account scope (`--account <name>` or `--all-accounts`). **If the user asks to generate/run a report but does not specify `daily_report` vs `portfolio_report`, stop before Phase A, do not run any report commands or live research, and ask which type they want with a brief explanation of both choices.** The selected type/scope controls which sections render and therefore which context/research work is allowed. **`portfolio_report` must not trigger §10.5 news search, event-calendar search, `research_targets`, `research_coverage`, high-risk/opportunity, recommendations/actions, holdings Action-column authoring, or `trading_psychology`; it is math/position focused.** `daily_report` keeps those daily decision sections, but skips Profit Panel, Performance Attribution, Discipline Check, Holding Period/Pacing, and P&L Ranking. `total_account` is scope, not a third type, and applies the math-only editorial suppression overlay.

Run the canonical pipeline in this order:

1. `python scripts/fetch_prices.py --account <name> --output "$REPORT_RUN_DIR/prices.json"` (or replace `--account <name>` with `--all-accounts`) — populates per-ticker latest prices and `prices.json["_fx"]` (§8 market-aware: **Stooq primary**, yfinance secondary; Binance/CoinGecko crypto; FX yfinance → Frankfurter/Open ER; Yahoo v8 currency verify; §9 auto-FX).
2. `python scripts/fetch_history.py --account <name> --merge-into "$REPORT_RUN_DIR/prices.json"` (or replace `--account <name>` with `--all-accounts`) — adds `_history` + `_fx_history` for snapshot math. This is price/history support, not news research. **Hard step:** if any required ticker / FX pair has zero rows after every cache + API fallback, the script exits 5 with a structured gap list. Fill only market-history gaps with `python scripts/fill_history_gap.py {ticker,fx} ... --merge-into "$REPORT_RUN_DIR/prices.json"`, then re-run fetch_history.py. Loop until exit 0. `--allow-incomplete` is debug-only — it must never be used for a deliverable report.
3. `python scripts/transactions.py snapshot --account <name> --prices "$REPORT_RUN_DIR/prices.json" --output "$REPORT_RUN_DIR/report_snapshot.json"` (or replace `--account <name>` with `--all-accounts`) — runs every numeric computation (`portfolio_snapshot.compute_snapshot`): aggregates, totals, base-currency conversion, book pacing, risk-heat scoring, §11 special checks, profit panel, realized + unrealized, transaction analytics. The snapshot is the single source of truth.
4. Author `$REPORT_RUN_DIR/report_context.json` with editorial-only content **only for sections included by the effective report policy**. Single-account `daily_report` may author `news`, `events`, `research_coverage`, `alerts`, `high_opps`, `adjustments`, `actions`, `trading_psychology`, theme/sector audit, Strategy readout, and reviewer notes. Single-account `portfolio_report` authors only non-daily context such as `theme_sector_html`, `theme_sector_audit`, `strategy_readout`, `data_gaps`, and `reviewer_pass`; it must omit `news`, `events`, `research_coverage`, `research_targets`, `high_opps`, `adjustments`, `actions`, `trading_psychology`, and holdings action text. Total-account scope may use empty/minimal context. Do **not** re-derive any numeric field that the snapshot already exposes. Before rendering, run `python scripts/validate_report_context.py --snapshot "$REPORT_RUN_DIR/report_snapshot.json" --context "$REPORT_RUN_DIR/report_context.json" --report-type <daily_report|portfolio_report> [--account-scope <single_account|total_account>]`.
5. `python scripts/generate_report.py --report-type <daily_report|portfolio_report> --account <name> --snapshot "$REPORT_RUN_DIR/report_snapshot.json" --context "$REPORT_RUN_DIR/report_context.json"` (or replace `--account <name>` with `--all-accounts`) — projects snapshot + context onto the policy-selected §10 HTML; embeds §13 popovers and §14 visual standard; uses stable EN / 繁中 / 简中 dictionaries via `scripts/i18n/*.json`. For non-built-in languages, translate `scripts/i18n/report_ui.en.json` to a file under `$REPORT_RUN_DIR` and pass via `--ui-dict`.
