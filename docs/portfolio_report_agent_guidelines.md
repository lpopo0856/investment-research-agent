...
| `render_high_risk_opp` / High Risk and High Opportunity | Render | Skip | Skip | Daily-only Phase B from snapshot risk heat + §10.5 decision evidence, authored in the SETTINGS user persona. Forbidden for portfolio/total. |
| `render_adjustments` / Recommended Adjustments | Render | Skip | Skip | Daily-only user-strategy recommendations with structured PM fields supplied for renderer math. Requires §10.5 evidence when action depends on external developments; PM best practice must not change the action unless SETTINGS supports it. Forbidden for portfolio/total. |
| `render_actions` / Today's Action List | Render | Skip | Skip | Daily-only user-persona action buckets from §10.5 + strategy. Reviewer/critic voice is forbidden in Phase B. Forbidden for portfolio/total. |
