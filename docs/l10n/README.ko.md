...
대만 주식 포지션은 가능하면 대만 증권거래소(TWSE) 내보내기 파일을 사용자에게 제공하도록 안내하세요.

PDF에 비밀번호가 걸려 있으면 브라우저에서 열고 브라우저 인쇄 기능으로 비밀번호 없는 PDF로 저장한 뒤 가져오라고 안내하세요.

거래 파일이 매우 크다면(특히 PDF) 더 작은 파일로 나누어 한 번에 한 배치씩 가져오라고 요청하세요.
...
데이터에 쓰기 전에 반드시 확인하고, `docs/` 아래 계약 문서를 처음부터 끝까지 따릅니다. 평소 말투로 요청하면 되고, 세부 실행은 에이전트가 처리합니다.

## 개인정보·로컬 보관

설정, 거래 데이터베이스(SQLite), 생성된 모든 리포트는 **로컬에만** 남으며 Git에 올라가지 않습니다. 버전 관리에는 에이전트 스펙, 예시 템플릿, Python 스크립트만 포함됩니다.
