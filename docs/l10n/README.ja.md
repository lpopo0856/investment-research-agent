...
- `scripts/generate_report.py`: 標準の HTML レンダラ。`report_snapshot.json` と検証済みの `report_context.json` を読み、レンダリング段階ではポートフォリオ数値を再計算しません。
- `reports/`: 出力先。ローカル専用。
...
エージェントは毎回作り直さず、標準スクリプトを使うべきです。取引データを読むステップは既定でルートの `transactions.db` を使います。
...
# 単一の数値スナップショットを生成します。profit panel、realized/unrealized、
# transaction analytics はこの snapshot に含まれます。
python scripts/transactions.py snapshot \
    --prices prices.json --settings SETTINGS.md --output report_snapshot.json

# エージェントは snapshot、最新公開情報、SETTINGS、guidelines から
# report_context.json を作成します。context には theme_sector_audit、
# research_coverage、trading_psychology、Strategy readout、reviewer_pass、
# actions / adjustments などの editorial フィールドが必須です。
...
`report_context.json` は `validate_report_context.py` を通る必要があります。旧 `style_readout` キーもレンダリングされますが、新しい context では `strategy_readout` を使ってください。
