...
**全口座をまたいだ合算レポートを作る（純粋に数値だけ）：**

> 「今日の合算レポートを出して。」
> 「全口座をひとつにまとめてレポートを作って。」

合算レポートは各口座のポジションと現金を統合し、同じ数学カーネルを通しますが、論評セクション（ニュース、イベント、アラート、アクションリスト、心理、テーマ／セクター…）はすべてスキップします。既定言語は `en`（ビルトイン：`en` / `zh-Hant` / `zh-Hans`）、既定基準通貨は `USD`、出力先は `accounts/_total/reports/` です。
