...
Sections below describe renderer capabilities. **Required** means required only when the effective policy (`report_type` + `account_scope`) renders that section; see the main guideline's "Section-level pipeline routing" table and `scripts/report_mode_policy.py`. Skipped sections must not trigger their data collection or context authoring.
...
2.7. **Trading-psychology evaluation (§10.1.7) (HARD REQUIRED WHEN RENDERED)** — agent-authored editorial section that reads as the user's own self-coaching note. Renders between transaction analytics and the holdings table so the flow is `evidence → reflection → positions`. The renderer is deterministic; content comes only from `context["trading_psychology"]`. Missing `context["trading_psychology"]` is a render-blocking error when `render_trading_psychology` renders, not a placeholder state. `portfolio_report` and `total_account` skip this section and must not author it.
   ...
   **Pipeline placement (mandatory gate when rendered)**: between `python scripts/transactions.py snapshot` and `python scripts/generate_report.py`, if `render_trading_psychology` is included, the agent must:
   ...
   **Authoring rules:**
   ...
   - Empty / missing `trading_psychology` is not acceptable when the section renders. If the evidence shows no behavior problem, still write a valid block with at least one observation that states the clean read and cites the relevant `snapshot.transaction_analytics` path, plus at least one improvement or maintenance rule anchored to `SETTINGS.md`. When policy skips the section, omit it.
...
### 10.5 News & event coverage (HARD WHEN RENDERED; agent owns web)

**Mode gate (HARD).** This section runs only when the effective report policy renders a daily decision/research section: `render_news`, `render_events`, `render_high_risk_opp`, `render_adjustments`, `render_actions`, `render_alerts`, or `render_today_summary`. Under the current policy this is **single-account `daily_report` only**. `portfolio_report` and all `total_account` variants **must not** run this workflow, must not derive `research_targets`, must not author `research_coverage`, and must not delegate a news/events temp-researcher. Their correct state is absent §10.5 context, not empty researched context.
...
Render gate when §10.5 renders: every cover-universe ticker has ≥1 news item or explicit `news_search` audit, and ≥1 30-day dated catalyst or explicit `event_search:<ticker>:no_dated_catalyst_within_30d`, represented in `research_coverage`. No model-memory catalyst dates. If search missing, `scripts/validate_report_context.py` fails before render. Every holding must either get evidence-backed §10.9/§10.10 recommendation, explicit `hold — no material news in search window` + audit, or `Need data`. Short-term coverage must support an immediate tactical state; mid/long coverage must support thesis/strategic status. Generic headline-only support returns to Phase A/Phase B. When §10.5 is skipped by policy, the validator must not require `research_coverage`.

#### 10.5.1 Final reply audit

Reply must name searched tickers and material-item count per ticker, including zero-count tickers, **only when §10.5 rendered**. For `portfolio_report` / `total_account`, omit this audit because no search should have run.

### 10.6 High-priority alerts (daily-only when rendered)

Render top alert block if any when `render_alerts` is included: single asset >20%; correlated theme >30%; high-vol bucket >30%; short-term position one-day move >8%; earnings within 7d and weight >5%; below 50dma plus negative news; price >20% above analyst consensus target; material negative news/guidance/regulatory/liquidity/dilution/debt risk regardless of weight; any §10.9 recommendation that breaches a SETTINGS rail and uses §15.6 path (c) conviction-reduced escalation.

---

## 11. Per-run special checks

Explicitly answer all 10 for rendered decision sections; for `portfolio_report`, answer math/snapshot-supported checks and mark news-dependent clauses as `not run by portfolio_report policy` rather than triggering §10.5. Clean pass still stated where applicable: single asset >15%; correlated theme >25%; high-vol bucket >30% (crypto, small-cap growth, unprofitable, abnormal vol); short-term overheated/rich/imminent event; losing positions price pullback vs fundamental/news/estimate deterioration; cash enough for 1-3m drawdown/adds; Short Term lot held >12m; ticker with 3+ new lots in 30d; latest lot cost >1.1× older weighted-average cost; any cost/date `?` gaps.
