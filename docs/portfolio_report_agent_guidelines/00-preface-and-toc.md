...
- Account resolution: use the user-named account when present; otherwise omitting `--account` resolves the active account from `accounts/.active`, falling back to `accounts/default/`. Ordinary single-account report requests use this active/default account; total/all-accounts scope is used only when explicitly requested. All pipeline inputs (`transactions.db`, `SETTINGS.md`, `reports/`) are read from `accounts/<active>/`.
