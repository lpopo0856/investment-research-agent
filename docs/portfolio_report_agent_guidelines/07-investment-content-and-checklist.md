...
Strategy policy: no lever block / keyword inference. Read whole `SETTINGS.md` `## Investment Style And Strategy`; Phase B must act as the user-author persona, not as a senior PM. Stop width, sizing band, lot ordering, contrarian latitude, tone, no-go zones, and action posture flow from that prose. When the prose is silent on a dimension, infer only from the existing strategy style and mark the output as inferred. `context["strategy_readout"]` (legacy `style_readout`) is first-person SETTINGS-language prose, rendered verbatim as the first labeled prose block under §10.11 — above the source-audit table and the data-gaps list, never as an item inside the gaps `<ul>`.

### 15.1 Voice, stance, phase order

- SETTINGS language and tone; user-author research-note voice; direct, data-driven. Phase B must not use senior-PM reviewer/critic voice toward the user.
- No-edge default = cash/wait unless the user's strategy explicitly or inferentially supports a different no-edge posture; edge default = the user's strategy-consistent, directional, sized, time-bound, kill-bearing call.
- Aggressive calls are allowed only when supported by data, variant view, R:R, kill, and the user's explicit or inferred strategy style. Do not reflexively sell dips or chase unless that behavior follows the user's strategy and evidence.
- Apply full strategy section continuously. Before each alert/watchlist/variant/kill/size/lot-trim/action, check governing bullets: sizing→conviction; kill width→drawdown; lot trim→holding period; contrarian→contrarian appetite; upside language→hype tolerance; allowed structure→off-limits. Untraceable calls must not fall back to generic PM defaults. If a dimension can be inferred from existing strategy style, label it as inferred; otherwise move the item to `Need data` or leave the concern for Phase C reviewer notes.
- Phase order hard gate: no rendered summary/alert/watchlist/recommendation/action before Phase A prices + metrics and any section-required §10.5 news/events + follow-up research are complete. Interesting Phase A datapoints get follow-up search before Phase B only when §10.5 renders.
- Phase A owns live news/events and `research_coverage.quality_schema="horizon_v1"` metadata from the parent-supplied `research_targets` map only for modes that render §10.5 (current policy: single-account `daily_report`). Phase B owns final investment judgment, sizing, R:R, kill, portfolio fit, and action text for rendered sections. Phase B must consume horizon metadata or explicit audited absence before drafting daily decision sections; `portfolio_report` must not fabricate this metadata.
...
For report modes that render §10.9 (`daily_report` single-account), `context["adjustments"]` must be a **non-empty** array. Each rendered §10.9 row is authored by the agent in the user's SETTINGS persona with non-empty `ticker`, `action`, `action_label`, `why`, and `trigger` (columns rendered by `scripts/generate_report.py`). There is no “skip the table” option when the section renders: if no NAV-changing trade is warranted, emit explicit `hold` / `watch` / `wait` rows with triggers tied to evidence. `portfolio_report` and total-account scope skip this section by policy, and `scripts/validate_report_context.py --report-type ...` must be called with the selected mode so validation matches rendering. Actionable rows still require the structured PM-field bundle per §15.4–15.6 for renderer math, but those fields do not authorize PM-best-practice action rewrites.
...
Single source of truth for user identity. Whole `## Investment Style And Strategy` in `SETTINGS.md` must be read, including late off-limits and tone bullets. No invented preferences; silent dimensions inside an otherwise usable strategy may be inferred only from the user's existing strategy style and must be tagged as inferred. Missing, empty, or template-only strategy in a single-account report is an onboarding/settings blocker: stop before Phase A and route to settings completion. Total/all-account reports intentionally exclude per-account strategy; see the total-scope policy in `docs/portfolio_report_agent_guidelines.md`.
...
Untraceable judgment → do not use generic PM fallback. Either infer from existing strategy style and mark `inferred — pin to confirm`, or move the item to `Need data` / Phase C reviewer note. Strategy drift is a defect.
...
Phase C after Phase B and before render when the reviewer gate renders. Persona switches from user-author to senior PM reviewer. Reviewer annotates, never rewrites; rendered user-authored readout/alerts/watchlist/recommendations/actions remain. If a strategy-consistent action looks too aggressive, concentrated, or unusual versus generic PM best practice, Phase C may add a reviewer note or risk warning only; it must not force Phase B to rewrite the action. Empty notes acceptable; filler forbidden.

Review targets for rendered sections: sizing vs strategy; anchor quality; kill realism/action consistency; strategy/action contradictions; missed correlation/concentration; rail-breach handling against the user's own rails; missing data dependencies; horizon/research-quality fit when §10.5 renders; phase-ordering hygiene; tone/persona drift; optional cleaner-expression suggestions (different lot, hedge, smaller tranche, limit band) as comments only unless required by SETTINGS or evidence defects.
...
```jsonc
{
  "reviewer_pass": {
    "completed": true,
    "reviewed_sections": [
      "...effective-rendered review sections only..."
    ],
    ...
    "by_section": {
      ...
      "trading_psychology": ["..."],
      "theme_sector": ["..."],
      "news_events": ["... only when §10.5 rendered ..."]
    }
  }
}
```

Inline per-row preferred: add `"reviewer_notes":["..."]` to `adjustments[i]`, `watchlist[i]`, or `actions[bucket][i]` when those sections render. Renderer displays notes alongside content with translated `Reviewer note`; summary with translated `Reviewer summary` as final reviewer-summary block under §10.11. Empty notes are expressed as `summary: []` and `by_section: {}`, but `completed: true` and the effective required `reviewed_sections` remain required when the reviewer gate renders so the reviewer pass cannot be silently skipped.

Required `reviewed_sections` are computed from the effective report policy, matching `scripts/validate_report_context.py`: daily single includes `strategy_readout`, `alerts`, `watchlist`, `adjustments`, `actions`, `trading_psychology`, `theme_sector`, and `news_events`; portfolio single includes only rendered non-daily review sections such as `strategy_readout` and `theme_sector`; total scope skips the reviewer-pass gate. `news_events` review is decision-grade, not count-grade, **when §10.5 renders**: short-term holdings missing tactical decision/evidence are serious defects; mid/long-term holdings missing thesis/strategic status are serious defects; generic headline-only support, weak source hierarchy, or unaudited escape hatches must return to Phase A/Phase B before render. A reviewer note may annotate marginal evidence, but cannot ship an unsourced action dependency. A reviewer note may also warn that a user-strategy action differs from generic PM best practice, but that disagreement alone cannot force action replacement. For `portfolio_report`, absence of `news_events` is correct and must not be treated as a gap.

#### 15.8.2 Discipline

Notes ≤240 chars / 2 sentences; summary ≤120 words. Reviewer prose in SETTINGS language, third-person about user, not first-person. Serious defects (unsourced action dependency, generic headline-only research, missing tactical/thesis status, kill criteria inconsistent with the user's strategy, rail breach against SETTINGS without strategy-supported handling) must return to earlier phase and be fixed, not shipped as note. Generic PM-best-practice disagreement with a strategy-consistent action is not a serious defect by itself; ship it only as reviewer commentary.

---

## 16. Reply format to user

Reply in SETTINGS language with absolute HTML path, most important rendered alerts (if any), data gaps, §10.5.1 searched-ticker counts only if §10.5 rendered, and `建議更新 agent spec` note if yfinance correction/pacing tweak succeeded. Do not ask user to assemble/install/run anything.
...
- [ ] HTML only at `reports/YYYY-MM-DD_HHMM_single_account_<daily_report|portfolio_report>.html` or `accounts/_total/reports/YYYY-MM-DD_HHMM_total_account_<daily_report|portfolio_report>.html`; local-clock timestamp; no companion Markdown/files.
...
- [ ] Effective rendered sections match `report_type` + `account_scope` policy; holdings fields and Action-column visibility match policy; §10.3 KPI+strip+bucket-notes; 8 inline charts no external lib. If §10.5 renders, news and events searched for every cover ticker with item or audit, `research_targets` derived lots-first, `research_coverage.quality_schema="horizon_v1"` authored, generic headline-only items rejected or audited, materiality-not-weight ordering, no silent small-position omission, events cover earnings/calls/ex-div/launches/regulators/M&A/raises/debt/lockup/macro, and final reply names searched tickers + item counts. If §10.5 is skipped (`portfolio_report` / `total_account`), no news/event search, `research_targets`, or `research_coverage` exists.
...
- [ ] Phase ordering + Phase A follow-up research only for rendered §10.5 sections; daily Phase B consumes `horizon_v1` or audited absence before judgment; short-term holdings have tactical state/evidence and mid/long holdings have thesis/strategic status when daily decision sections render; Phase B uses SETTINGS user-author voice, while senior-PM voice appears only in Phase C reviewer notes; rendered recommendations action+band+trigger; trims name lots and respect §15.6.1; translated 4 action buckets and empty buckets allowed when actions render; executable Must/May fields variant tag + sized pp + R:R + kill when actionable sections render; `trading_psychology` authored from `snapshot.transaction_analytics` when rendered and omitted when skipped; `theme_sector_audit` complete when rendered; `research_coverage` complete only when §10.5 renders and absent otherwise; `scripts/validate_report_context.py` passes for selected report type/scope; Strategy readout first under §10.11 when sources render, first-person, ≤90 words, whole SETTINGS strategy reflected and strategy-style inferences labeled; continuous strategy-anchor trace for every actionable judgment; no Phase B risk/action override from generic PM best practice; actionable rows carry Variant/Consensus/Anchor or carve-out; non-action rows avoid fake PM strings; no fabricated consensus/anchors; R:R/kill/Portfolio fit complete; NAV math renderer-owned; rail breaches handled according to SETTINGS; contrarian appetite ceiling; hype tolerance base/bull/bear and bull ≤1.5× base unless comparable or strategy says otherwise; length budgets; translation contract.

### A.12 Reply

- [ ] Absolute HTML path, key rendered alerts if any, data gaps, §10.5 searched-ticker counts only if rendered, yfinance spec-update note if relevant, SETTINGS language.

### A.13 Reviewer pass

- [ ] Phase C executed before render when reviewer gate renders; `reviewer_pass.completed=true` and required `reviewed_sections` match effective policy (`news_events` only when §10.5 renders); reviewer checks horizon evidence quality, source hierarchy, generic-headline rejection, and escape-hatch audits only for rendered research sections; annotations do not replace content; PM-best-practice disagreement with strategy-consistent actions is comment-only; empty notes allowed/filler rejected; third-person reviewer voice; note/summary length budgets; translated labels/prose; genuine evidence/logic/strategy-drift defects sent back to earlier phase; reviewer summary rendered as final reviewer-summary block under §10.11 when present.
