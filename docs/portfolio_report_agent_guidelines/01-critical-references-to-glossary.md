...
- Do **not** edit or delete `SETTINGS.md` or `ledger/` (resolved from `accounts/<active>/` via `--account <name>` or `accounts/.active`) unless the user explicitly asks.
...
| Active ledger store | Canonical account Markdown ledger at `accounts/<active>/ledger/`, with append-only `events/` and rebuildable `generated/` caches. Resolved via `--account <name>` or the `accounts/.active` pointer (fallback: `accounts/default/`). Auto-read fresh every run via store helpers. If you suspect ledger drift, rebuild/verify generated caches. Do not hard-code holdings. |
| `SETTINGS.md` | Output language, tone, optional sizing rails, and the full `## Investment Style And Strategy` section. Resolved from `accounts/<active>/SETTINGS.md` via `--account <name>` or the `accounts/.active` pointer. |
...
| 5 | A · Gather | For tickers still unresolved, walk the source hierarchy: web search / public quote pages → no-token public endpoints | §8.1, §8.5, §8.6 |
...
| 12 | **C · Review** | **Switch persona** from "I am the user" to a senior PM reviewing the user's analysis. **Re-read the full `## Investment Style And Strategy`** as the touchstone for the review. Review only rendered strategy-dependent sections required by the effective policy (daily: alerts / watchlist / recommendations / actions / trading psychology / Strategy readout / news-events evidence; portfolio: Strategy readout and theme/sector where rendered; total: skipped). Attach **review notes** where specific; do not replace user content. If the only concern is that a strategy-consistent action is too aggressive/concentrated/unusual versus generic PM best practice, comment or warn only; do not force a more conservative rewrite. Empty notes are acceptable and filler is forbidden | §15.7, §15.8 |
...
|---|---|
| A → B | `ledger/` and `SETTINGS.md` from `accounts/<active>/` (via `--account` or `accounts/.active`) and required docs read; positions loaded via the ledger store helper and classified; `$REPORT_RUN_DIR/prices.json` generated or exhaustive fallback audit captured; FX rates resolved or marked `n/a`; metrics computed. If §10.5 is rendered, every cover-universe ticker has news search status and event search status and follow-up searches are complete. If §10.5 is skipped, `news` / `events` / `research_targets` / `research_coverage` are absent by design. |
| B → C | All rendered Phase B sections are drafted from the evidence required by those sections and in the SETTINGS user-author persona. Daily single requires high-priority alerts, high-risk / high-opportunity watchlist, recommended adjustments, action list, `trading_psychology`, Strategy readout, and today's summary. Portfolio single requires Strategy readout, theme/sector audit, and data gaps only. Total scope skips strategy-dependent editorial gates. Continuous strategy-anchor check completed for rendered judgments; strategy-style inferences are marked as inferred; no reviewer/critic voice appears in Phase B. |
| C → D | Reviewer pass completed in senior-PM voice for rendered review sections only; item-level notes attached only where specific; genuine evidence/logic/strategy-drift defects returned to Phase A or B and fixed; generic PM-best-practice disagreement with a strategy-consistent action remains comment-only; `python scripts/validate_report_context.py --snapshot "$REPORT_RUN_DIR/report_snapshot.json" --context "$REPORT_RUN_DIR/report_context.json" --report-type <selected> [--account-scope <selected>]` passes; no reviewer note is being used to paper over missing data, gated research gaps, theme gaps, strategy violations, invalid kill criteria, or weak psychology evidence. |
...
**Anti-pattern (HARD) — Phase C skipped.** Going straight from user-voice analysis to render — without the senior-PM reviewer pass — produces reports that lock in unchallenged blind spots. The user-as-author voice is too close to its own analysis to catch sizing inconsistencies, weak anchors, kill criteria that contradict the user's own risk setup, or contradictions between the Strategy readout and the action list. The reviewer hat is what surfaces those as notes or genuine defects. Even a "no notes — content is sound" output from Phase C is required; silently skipping the pass is a violation.

**Anti-pattern (HARD) — strategy-anchor drift.** Reading `## Investment Style And Strategy` once at session start and then drafting Phase B / Phase C content from a fading memory of "what the user roughly wanted" produces calls that quietly diverge from the user's stated stance — typically by adopting PM defaults under the pull of a compelling standalone analysis. The full strategy text must remain your active touchstone for every actionable judgment; re-read the relevant bullets before each call. A Phase B judgment that cannot be traced to an explicit strategy bullet may be inferred only from the user's existing strategy style and must be labeled as inferred; otherwise it is PM drift and must move to `Need data` or the reviewer note, not into the action itself.
...
| Term | Meaning |
|---|---|
| **Lot** | A projected open position from the active ledger replay/cache: ticker, quantity, cost basis, acquisition date, bucket, market, currency. The view/cache is rebuilt from fresh canonical replay after every confirmed transaction write. |
...
| **Source hierarchy** | The fallback order for latest prices. **Listed securities:** Stooq JSON → `yfinance` per-ticker → web search / quote pages → other no-token public endpoints. **FX:** `yfinance` `=X` → Frankfurter / Open ER / official central-bank reference → web pages if needed. **Crypto:** Binance / CoinGecko → web pages → other no-token public endpoints. After every Stooq hit, the ticker quote currency is verified via Yahoo's v8 chart endpoint |
...
| **Strategy-anchor check** | The continuous-reference discipline: the full `## Investment Style And Strategy` content must remain the active touchstone for every Phase B / Phase C judgment, with each call traced back to specific strategy bullets (sizing → conviction; kill width → drawdown; lot-trim → holding-period; contrarian variant → contrarian appetite; upside cap → hype tolerance; allowed asset / structure → off-limits zones). Internalisation is not a one-time read. If the strategy is silent, Phase B may infer from the existing strategy style only when the output labels the dimension as inferred (§15.7) |
| **Reviewer pass** | Phase C — the agent switches hat from "I am the user" to a senior PM reviewing the user's analysis, then walks every alert / watchlist / recommendation / action item and attaches review notes where it has something to flag or suggest. Does not replace the user's content, and does not force PM-best-practice rewrites of strategy-consistent actions (§15.8) |
